...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch fresh price from RapidAPI
    const options = {
      method: 'GET',
      headers: {
    ...
    const response = await fetch(
      `https://${process.env.RAPIDAPI_HOST}/goat-search?query=${encodeURIComponent(listing.productSKU)}`,
      options
    );
    ...
    // Parse the price from response (structure depends on actual API response)
    // You'll need to adjust this based on the actual response format
    const price = data.results?.[0]?.lowestAsk || data.results?.[0]?.retailPrice || null;
